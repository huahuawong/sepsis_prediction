...
from utility import save_pickle, compute_prediction_utility, TimeSeriesDataset
import torch
...
# File locations for the training dataset
locations = [input_directory + x for x in ['training_setA/training', 'training_setB/training']]

# Ready to store and concat the two training data files
data = []

# Make dataframe before saving it into a pkl file. The hospital ids and patient ids are also denoted
...
save_pickle(df, input_directory + '/raw/df.pickle')
...
def create_timeseries_dataset(df):
    """ Creates a utility.TimeSeriesDataset from the pandas dataframe. """
...
# Save the data
save_pickle(scores, input_directory + '/processed/labels/full_scores.pickle')
save_pickle(utility_scores, input_directory + '/processed/labels/utility_scores.pickle')
save_pickle(binary_labels, input_directory + '/processed/labels/binary.pickle')
save_pickle(overall_labels, input_directory + '/processed/labels/overall_labels.pickle')
dataset.save(input_directory + '/raw/data.tsd')
